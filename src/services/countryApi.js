...
export const countryService = {
  ...
  async getAllCountries() {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/all?fields=name,capital,population,area,currencies,languages,flags,cca2,region,subregion`);
      if (!response.ok) {
  ...
  processCountriesData(apiData) {
    ...
    apiData.forEach(country => {
      ...
      countries[countryId] = {
        ...
        description: {
          en: `${countryName} is located in ${country.region}${country.subregion ? `, ${country.subregion}` : ''}.`,
          de: `${countryName} befindet sich in ${country.region}${country.subregion ? `, ${country.subregion}` : ''}.`,
          fa: `${this.getPersianName(countryName)} در ${country.region} قرار دارد.`
        },
